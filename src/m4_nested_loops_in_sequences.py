...
def multiply_numbers(sequence_of_lists):
    ...
    # ------------------------------------------------------------------
    for a in range(1, 3):
        for b in range(len(sequence_of_lists[a])):
            sequence_of_lists[a][b] = sequence_of_lists[a][b] * (a + 1)
...
def run_test_sum_numbers():
    ...
    expected = 9
    answer = sum_numbers(([1, 2, 4], [5, 7], [-10]))
    print('Expected and actual are:', expected, answer)
...
def print_characters(sequence_of_strings):
    ...
    #  ** ASK QUESTIONS if you do not understand the TESTS (ABOVE).
    # ------------------------------------------------------------------
    for a in range(len(sequence_of_strings)):
        for b in range(len(sequence_of_strings[a])):
            print(sequence_of_strings[a][b])
...
def print_characters_slanted(sequence_of_strings):
    ...
    for a in range(len(sequence_of_strings)):
        for b in range(len(sequence_of_strings[a])):
            print(' ' * b + sequence_of_strings[a][b])
